...
pub struct QuadApp {
    ...
    h_pole_mm:  f64,   // pole piece height
    ...
    a_mag_mm2:  f64,   // magnet cross section mm²
...
impl QuadApp {
    ...
    fn draw_inputs(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("magnet_grid")
            ...
            .show(ui, |ui| {
                ...
                ui.label("Pole height (mm)");
                ui.add(egui::Slider::new(&mut self.h_pole_mm, 10.0..=200.0).step_by(1.0));
                ui.end_row();
                ...
                ui.label("Magnet area (mm²)");
                ui.add(egui::Slider::new(&mut self.a_mag_mm2, 100.0..=20000.0).step_by(100.0));
                ui.end_row();
    ...
    fn draw_tab_results(&self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("opt_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("MMF₁ (outer)");
        ...
        egui::Grid::new("spot_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("Spot x");
            ...
            ui.colored_label(
                if imbal > 10.0 { egui::Color32::YELLOW } else { egui::Color32::WHITE },
                format!("{:.1} %", imbal),
            );
            ui.end_row();
        });
        ...
        egui::Grid::new("env_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("Max env x");
            ui.colored_label(
        ...
        egui::Grid::new("btip_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("B_tip outer");
            ui.colored_label(
                if b1 > self.b_sat * 0.9 { egui::Color32::RED }
                else if b1 > self.b_sat * 0.7 { egui::Color32::YELLOW }
            ...
            ui.colored_label(
                if b2 > self.b_sat * 0.9 { egui::Color32::RED }
                else if b2 > self.b_sat * 0.7 { egui::Color32::YELLOW }
                else { egui::Color32::WHITE },
                format!("{:.3} T", b2),
            );
            ui.end_row();
        ...
        egui::Grid::new("phys_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("Bρ");
    ...
    fn draw_tab_fluxes(&self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("flux1_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("φ total");
        ...
        egui::Grid::new("flux2_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("φ total");
    ...
    fn draw_tab_energies(&self, ui: &mut egui::Ui) {
        ui.label(egui::RichText::new("Outer quads (Q1 / Q3)").strong());
        egui::Grid::new("energy1_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("E total");
        ...
        ui.label(egui::RichText::new("Inner quad (Q2)").strong());
        egui::Grid::new("energy2_grid").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
            ui.label("E total");
    ...
    fn draw_plot(&self, ui: &mut egui::Ui) {
        ...
        let x_pos: PlotPoints = t.z.iter().zip(t.x.iter())
            .map(|(&z, &x)| [z * 1000.0, x * 1000.0]).collect();
        let x_neg: PlotPoints = t.z.iter().zip(t.x.iter())
            .map(|(&z, &x)| [z * 1000.0, -x * 1000.0]).collect();
        let y_pos: PlotPoints = t.z.iter().zip(t.y.iter())
            .map(|(&z, &y)| [z * 1000.0, y * 1000.0]).collect();
        let y_neg: PlotPoints = t.z.iter().zip(t.y.iter())
            .map(|(&z, &y)| [z * 1000.0, -y * 1000.0]).collect();
            ...
            .show(ui, |plot_ui| {
                ...
                for (z_start, z_end, label) in quad_boundaries {
                    ...
                    plot_ui.line(Line::new(PlotPoints::new(vec![
                        [z_start, -bore_mm * 1.2],
                        [z_start,  bore_mm * 1.2],
                    ])).color(quad_col).width(1.5).name(label));

                    // Right edge
                    plot_ui.line(Line::new(PlotPoints::new(vec![
                        [z_end, -bore_mm * 1.2],
                        [z_end,  bore_mm * 1.2],
                    ])).color(quad_col).width(1.5));
